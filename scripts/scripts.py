...
with open('2017PythonData.csv', 'r') as csv_file:
    ...
    for line in csv_file:
        if (nextline == True) :
            ...
            nextline = False
        #We are assuming that people use spaces properly
        words = line.split(' ')
        for word in words:
...
# move the if statement into the innermost for loop, the for word in words loop.
